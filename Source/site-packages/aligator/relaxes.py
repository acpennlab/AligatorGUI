...
import copy
import numpy
from aligator.mechanisms import mechanism
#from qmat import Q_mat
from aligator.core_utils import generate_Q, r_tuple_from_r_name, convert, convert_ms
...
class ExperimentSetup:
    ...
    def curve_save(self, curve_data, mod=""):
        ...
        #interpolates
        currenty=self.trace_coll[:,0]
        timex=self.trace_coll[:,1]
        gluty=self.trace_coll[:,2]
        dx = 2e-5
        timexl = numpy.arange(min(timex),max(timex),dx)
        
        currentyl = numpy.interp(timexl,timex,currenty)
        glutyl = numpy.interp(timexl,timex,gluty)
        
        self.trace_coll_linear = numpy.column_stack((currentyl,timexl,glutyl))
        
        #saves in txt form
        if self.header:
            print (self.header)
            numpy.savetxt(self.param.sim_name + mod + "_linear.txt", self.trace_coll_linear, delimiter='\t', header=self.header)
        else:
            numpy.savetxt(self.param.sim_name + mod + "_linear.txt", self.trace_coll_linear, delimiter='\t')

        #saves in atf form
        atflin = open(self.param.sim_name + mod + "_linear.atf", "w")
        atflin.write("ATF\t1.0\n0\t3\nTime (ms)\tTrace\tAgonist (au)\n")
        atftext = self.trace_coll_linear.tolist()
        for row in atftext:
            atflin.write(str(row[1]*1000)+'\t')
            atflin.write(str(row[0]*1000)+'\t')
            atflin.write(str(row[2]*1000)+'\n')
        atflin.close()            
        #saves in itx (igor) form
        itxlin = open(self.param.sim_name + mod + "_linear.itx", "w")
        itxlin.write("IGOR\nWAVES\tTrace\tAgonist\nBEGIN\n")
        itxtext = self.trace_coll_linear.tolist()
        for row in itxtext:
            itxlin.write(str(row[0]*1000)+'\t')
            itxlin.write(str((row[2]-1.05)*1000)+'\n')
        itxlin.write('END\nX SetScale/P x 0,0.02, Trace\nX SetScale/P x 0,0.02, Agonist\nX Display Trace,Agonist\nX Label bottom "Time (ms)"\nX Label left "Response (au)"')
        itxlin.close()
    ...
    def text_out(self, extra_data=False):
        ...
        self.f=open(self.param.sim_name + 'rate.txt', 'w')
